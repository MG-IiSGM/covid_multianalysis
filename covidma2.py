    ...
    for i in range(len(r1)):
        ...
        if sample in sample_list_F:
            ...
            if not os.path.isfile(output_markdup_trimmed_file):
                os.system("sbatch /home/laura/Laura_intel/Desktop/covid_multianalysis/sample_mapper.sh %s %s %s %s %s %s %s > jobid.batch" 
                        %(output, r1_file, r2_file, sample, reference, threads, primers))
            else:
            ...
            if os.path.isfile(out_ivar_variant_file):
                ...
                if os.path.exists("jobid.batch"):
                    ...
                    os.system("sbatch --dependency=afterok:%s /home/laura/Laura_intel/Desktop/covid_multianalysis/ivar_variant_calling.sh %s %s %s %s %s > jobid.batch" 
                        %(jobid, output, output_markdup_trimmed_file, sample, reference, annotation))
                else:
                    os.system("sbatch /home/laura/Laura_intel/Desktop/covid_multianalysis/ivar_variant_calling.sh %s %s %s %s %s > jobid.batch" 
                        %(output, output_markdup_trimmed_file, sample, reference, annotation))
            ...
            if os.path.isfile(out_ivar_filtered_file):
                ...
                if os.path.exists("jobid.batch"):
                    ...
                    f.close()
                    os.remove("jobid.batch")
                    os.system("sbatch --dependency=afterok:%s /home/laura/Laura_intel/Desktop/covid_multianalysis/variant_filtering.sh %s %s %s > jobid.batch" 
                        %(jobid, output, sample, out_ivar_variant_file))
                else:
                    os.system("sbatch /home/laura/Laura_intel/Desktop/covid_multianalysis/variant_filtering.sh %s %s %s > jobid.batch" 
                        %(output, sample, out_ivar_variant_file))
            ...
            if os.path.isfile(out_ivar_consensus_file):
                ...
                if os.path.exists("jobid.batch"):
                    ...
                    f.close()
                    os.remove("jobid.batch")
                    os.system("sbatch --dependency=afterok:%s /home/laura/Laura_intel/Desktop/covid_multianalysis/consensus_ivar_create.sh %s %s %s > jobid.batch" 
                        %(jobid, output, sample, output_markdup_trimmed_file))
                else:
                    os.system("sbatch /home/laura/Laura_intel/Desktop/covid_multianalysis/consensus_ivar_create.sh %s %s %s > jobid.batch" 
                        %(output, sample, output_markdup_trimmed_file))
            ...
            if os.path.isfile(out_bamstats_file):
                ...
                if os.path.exists("jobid.batch"):
                    ...
                    f.close()
                    os.remove("jobid.batch")
                    os.system("sbatch --dependency=afterok:%s /home/laura/Laura_intel/Desktop/covid_multianalysis/bamstats.sh %s %s %s %s > jobid.batch" 
                        %(jobid, output, sample, output_markdup_trimmed_file, threads))
                else:
                    os.system("sbatch /home/laura/Laura_intel/Desktop/covid_multianalysis/bamstats.sh %s %s %s %s > jobid.batch" 
                        %(output, sample, output_markdup_trimmed_file, threads))
            ...
            if os.path.isfile(out_coverage_file):
                ...
                if os.path.exists("jobid.batch"):
                    ...
                    f.close()
                    os.remove("jobid.batch")
                    os.system("sbatch --dependency=afterok:%s /home/laura/Laura_intel/Desktop/covid_multianalysis/coverage_stats.sh %s %s %s > jobid.batch" 
                        %(jobid, output, sample, output_markdup_trimmed_file))
                else:
                    os.system("sbatch /home/laura/Laura_intel/Desktop/covid_multianalysis/coverage_stats.sh %s %s %s > jobid.batch" 
                        %(output, output, sample, output_markdup_trimmed_file))
                os.remove("jobid.batch")
            
            # CHECK PARALELL
            os.system('while [ "$(squeue | grep $USER | grep "mapper" | wc -l)" -ge "60" ]; do sleep 0.1; done')
            os.system('if [ %s = %s ]; then while [ $(squeue | grep $USER | grep "mapper" | wc -l) != 0 ]; do sleep 0.1; done; fi' %(str(counter), l))
            counter += 1
