...
def consensus_create(output, sample, output_markdup_trimmed_file):
    ...
    if os.path.isfile(out_ivar_consensus_file):
        print( out_ivar_consensus_file +
                    " EXIST\nOmmiting Consensus for  sample " + sample )
    else:
        print( "Creating consensus with ivar in sample " + sample )
...
def coverage_stats(output, sample, output_markdup_trimmed_file):
    ...
    if os.path.isfile(out_coverage_file):
        ...
        print( "Creating coverage in sample " + sample )
        # Compute coverage metrics using samtools
        create_coverage(output_markdup_trimmed_file,
                        out_stats_coverage_dir, sample)

def map_sample(output, r1_file, r2_file, f_sample_list_F, f_new_samples, reference, threads, primers, annotation):
    """
    ...
    if sample in sample_list_F:
        ...
        if sample in new_samples:
            ...
            print("\n" + "STARTING SAMPLE: " + sample +
                        " (" + sample_number + "/" + sample_total + ")" + " (" + new_sample_number + "/" + new_sample_total + ")" )
        else:
            print("\n" + "STARTING SAMPLE: " + sample +
        ...
        if not os.path.isfile(output_markdup_trimmed_file):
            ...
            print( output_markdup_trimmed_file +
                    " EXIST\nOmmiting BAM mapping and BAM manipulation in sample " + sample )
...
primers = sys.argv[8]
annotation = sys.argv[9]

# function
map_sample(output, r1_file, r2_file, sample_list_F, new_samples, reference, threads, primers, annotation)
