...
######### VERSION MOVIENDO ARCHIVOS A LA CARPETA UNCOVERED/ ##########
...
def remove_low_quality(output_dir, group, min_coverage=70, max_unmap=10):

# Mirar la función comentada de arriba para mover los archivos de las muestras
# de mala calidad a la carpeta Uncovered/ 
    output_dir = os.path.abspath(output_dir)
    ...
    for root, _, files in os.walk(output_dir):
        if root.endswith('Stats'):
            for name in files:
                ...
                if name.endswith('overal.stats.tab'):
                    coverage_stat_file = filename
                    filtered_coverage_stat_file = os.path.join(root, group + '_covered.stats.tab')
                    stats_df = pd.read_csv(coverage_stat_file, sep="\t")
                    # Store samples under any of the parameters indicated
                    uncovered_samples = stats_df['#SAMPLE'][(stats_df['COV>30X'] < min_coverage) |
                                                            (stats_df['UNMMAPED_PROP'] > max_unmap)].tolist()
                    ...
                    uncovered_df = stats_df[stats_df['#SAMPLE'].isin(
                        uncovered_samples)]
                    uncovered_table_filename = group + '_uncovered.stats.tab'
                    uncovered_table_file = os.path.join(
                        root, uncovered_table_filename)
                    if len(uncovered_samples) > 0:
                        uncovered_df.to_csv(
                            uncovered_table_file, sep="\t", index=False)
...
def check_reanalysis(output_dir, samples_to_analyze):
    ...
    variant_dir = os.path.join(output_dir, "Bam")
    ...
    if file_exist > 0:  # Already analysed
        ...
        previous_samples_list = [x.split(".")[0] for x in os.listdir(
            variant_dir) if x.endswith(".bam")]
