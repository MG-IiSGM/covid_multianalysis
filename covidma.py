...
def pangolin_annot(output, logger, nproc):
    """
                    ...
                    if name.endswith('.fa'):
                        ...
                        if os.path.isfile(out_pangolin_file):
                            ...
                            future = executor.submit(
                                annotate_pangolin, filename, out_annot_pangolin_dir, out_pangolin_filename, threads=nproc, max_ambig=0.6)
...
def covidma_pipeline(output, args, logger, r1, r2, sample_list_F, new_samples, group_name, reference, annotation):
    ...
    for r1_file, r2_file in zip(r1, r2):
        to_write = "python map_sample.py %s %s %s %s %s %s %s %s\n" %(output, primers, r1_file, r2_file, sample_list_F, new_samples, reference, annotation)
        f.write(to_write)
    f.close()
    os.system("paralell -j %s < %s" %("8", f))
    ...
    pangolin_annot(output, logger, nproc)
...
primers = os.path.abspath(args.primers)             # Primers path (argument)
# LOGGING
...
# Run COVIDMA
covidma_pipeline(output, args, logger, r1, r2, sample_list_F, new_samples, group_name, reference, annotation)
